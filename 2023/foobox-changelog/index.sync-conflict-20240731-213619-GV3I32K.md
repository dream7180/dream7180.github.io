...
{{< admonition success "2024-7-22, 7.31版" >}}
* 播放列表管理器支持 Control, Shift 或 Control+A 多选进行操作。
* 修正电台地址，为避免出现问题，电台列表地址和音频外部编辑器的配置对调了储存位置，请覆盖升级的朋友到 foobox 设置里重置一下这两个设定。
* 修正已知 bugs 以及优化代码提高效率。
* 更新 ESLyric 组件。
{{< /admonition >}}
{{< admonition success "2024-7-05, 7.30版" >}}
